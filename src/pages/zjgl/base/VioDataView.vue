     ...
     >
        <el-card class="box-card" style="padding-bottom:110px;">
          <el-col :span="12">
            <pic-zoom
              :url="srcList[0]"
              :scale="AppInfo.spe == 0 ? AppInfo.scale : (AppInfo.scale * 2)"
              :oheight="imgh"
              class="myimg1"
            >
              ...
              <template v-slot:wfxx>
                <div>{{formdata.WFSJ + " " + formdata.WZXWNO + " " +formdata.WFDZ}}</div>
                <div>{{formdata.HPHM + " " + formdata.CGCLPP + " " + formdata.JDCSYR}}</div>
                <el-form :inline="true" class="demo-form-inline">
                  <el-button size="mini" type="primary" icon="el-icon-edit-outline" @click="onShtg()" v-if="formdata.XH">审核通过</el-button>
                  <el-button size="mini" type="primary" icon="el-icon-s-release" @click="onSqShbtg()" v-if="formdata.XH">申请审核不通过</el-button>
                </el-form>
              </template>
            </pic-zoom>
          </el-col>
          <el-col :span="12">
            <pic-zoom
              :url="srcList[1]"
              :scale="AppInfo.spe == 1 ? AppInfo.scale : (AppInfo.scale * 2)"
              :oheight="imgh"
              class="myimg2"
            >
          ...
          <el-col :span="12">
            <pic-zoom
              :url="srcList[2]"
              :scale="AppInfo.spe == 2 ? AppInfo.scale : (AppInfo.scale * 2)"
              :oheight="imgh"
              class="myimg3"
            >
          ...
          <el-col :span="12">
            <pic-zoom
              :url="srcList[3]"
              :scale="AppInfo.spe == 3 ? AppInfo.scale : (AppInfo.scale * 2)"
              :oheight="imgh"
              :class="AppInfo.imgs==4 ? 'myimg4' : 'myimgno'"
              :csys="'品牌:'+formdata.CGCLPP + '   颜色:' + formdata.CGCSYS"
              :syr="'所有人:'+formdata.JDCSYR"
              :hphm="`号牌号码:${formdata.HPHM}`"
              :wfsj="`号牌号码:${formdata.WFSJ}`"
              :wfxw="formdata.WZXW"              
            >
              <template v-slot:header v-if="AppInfo.spe == 4" >
                <span style="color:#ffffff;font-size: 30px;font-weight: 700;background-color: darkblue;position:absolute;bottom:100%; ">{{formdata.HPHM}}</span>
              </template>
              <template v-slot:wfxx>
                <div>{{formdata.WFSJ + " " + formdata.WZXWNO + " " +formdata.WFDZ}}</div>
      ...
      <el-row>
        <el-col :span="24">
          <el-form :inline="true" :model="queryTmp">
            <el-col :xs="12" :sm="12" :md="24" :lg="9" :xl="9">
...
export default {
  ...
  watch: {
    tabidx (nval) {
      if (nval === -1) return
      this.srcList = []
      this.srcList.push(path[this.AppInfo.imgroot] + this.tableData[nval].IMG1)
      this.srcList.push(path[this.AppInfo.imgroot] + this.tableData[nval].IMG2)
      this.srcList.push(path[this.AppInfo.imgroot] + this.tableData[nval].IMG3)
      this.srcList.push(path[this.AppInfo.imgroot] + this.tableData[nval].IMG4)
  ...
  methods: {
    ...
    onShtg () {
      this.$confirm('是否审核通过?', '提示', {
        ...
        this.$axiosClient.call(this.AppInfo.shtg, 'POST', {xh: this.formdata.XH, bklb: '4', firstimgidx: this.AppInfo.firstimgidx})
          .then((res) => {
    ...
    sqshbtg (sqyy) {
      ...
      let i = this.source.findIndex(d => d.XH === this.formdata.XH)
      this.source[i].HDBJ = 'p'
      this.$axiosClient.call(this.AppInfo.sqshbtg, 'POST', {xh: this.formdata.XH, sqyy: sqyy, bklb: '4',firstimgidx: this.AppInfo.firstimgidx})
        .then((res) => {
          this.source[i].HDBJ = res.hdbj
          if (res.status === 0) {
            this.$message.error(res.msg)
    ...
    next (start) {
      if (start < this.tableData.length) {
        if (this.tableData[start].HDBJ === this.AppInfo.hdbj) {
          return start
        } else {
...
<style scoped>
  ...
  .myimgno::after {
    ...
    white-space: pre;
    width: 100%;
    content:attr(hphm) '\a' attr(syr) '\a' attr(csys) '\a' attr(wfxw);
  }
  .buttonactive {
    border-color: #b7d1e5;
