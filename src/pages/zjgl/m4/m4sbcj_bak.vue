          ...
          <el-col :span="12">
            ...
            >
              <template v-slot:header >
                <span style="color:#ffffff;font-size: 30px;font-weight: 700;background-color: darkblue;">{{formdata.HPHM}}</span>
              </template>
            </pic-zoom>
          </el-col>
          <el-col :span="12">
...
export default {
  ...
  mounted () {
    this.imgwidth = document.querySelector('.myimg1').clientWidth
  ...
  methods: {
    ...
    onShtg () {
      this.$confirm('是否审核通过?', '提示', {
        ...
        axiosClient.call('ZJGLM4SBCJSHTG', 'POST', {xh: this.formdata.XH, bklb: '4', firstimgidx: 1})
          .then((res) => {
    ...
    sqshbtg (sqyy) {
      ...
      let i = this.source.findIndex(d => d.XH === this.formdata.XH)
      this.source[i].HDBJ = 'p'
      axiosClient.call('ZJGLM4SBCJSQSHBTG', 'POST', {xh: this.formdata.XH, sqyy: sqyy, bklb: '4', firstimgidx: 1})
        .then((res) => {
          this.source[i].HDBJ = res.hdbj
          if (res.status === 0) {
            this.$message.error(res.msg)
