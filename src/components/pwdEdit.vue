...
export default {
  ...
  data () {
    let validatePass = (rule, value, callback) => {
      let regx = /^(?=.*\d)(?=.*[a-zA-Z])(?=.*[~!@#$%^&*])[\da-zA-Z~!@#$%^&*]{8,}$/;
      if (value === '') {
        callback(new Error('请输入密码'))
      } if (!regx.test(value)) {
        callback(new Error('必须包含数字、英文字母、特殊符号且大于等于8位'))
      } else {
    ...
    onSubmit (formName) {
      this.$refs[formName].validate((valid) => {
        if (!valid) {
          ...
          console.log(33333)
          axiosClient.call('sys/qxgl/user/editpwd', 'post', {id: this.user.ID, oldpwd: this.formdata.ymm, newpwd: this.formdata.xmm1})
            .then((res) => {
              if (res.status === 1) {
                this.$message('修改成功')
                this.$emit('hide')
